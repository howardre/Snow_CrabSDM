...
mat_female_train <- crab_train %>%
  dplyr::select(depth, mean_temp, phi, ice_mean,
...
grid_development <- function(train_data){
  ...
  spatial_grid$mean_temp <- median(train_data$mean_temp, na.rm = T)
...
summary(mat_female_gam_abun) # 36.9%
...
mat_female_tweedie <- gam(mature_female + 1 ~ s(year, bs = 're') +
                          ...
                          family = tw(link = "log"),
                          method = "REML")
summary(mat_female_tweedie) # 64.4%
...
leg_male_gam_base <- gam(pres_leg_male ~ s(year, bs = "re") +
                         ...
                         data = leg_male_train,
                         family = "binomial")
summary(leg_male_gam_base) # 54.7% explained
...
summary(leg_male_gam_abun) # 44.2%
...
vars <- c(1:8, 12) # including year as factor as done in Brodie et al., 2019
...
saveRDS(brt_mat_female_abun, file = here('data', 'brt_mat_female_abun_for.rds'))
saveRDS(brt_mat_female_base, file = here('data', 'brt_mat_female_base_for.rds'))

# Read in BRTs
brt_mat_female_abun <- readRDS(file = here('data', 'brt_mat_female_abun_for.rds'))
brt_mat_female_base <- readRDS(file = here('data', 'brt_mat_female_base_for.rds'))

# Variable names
column_names <- c("depth", "temperature", "phi", "ice_mean", "longitude",
                  "latitude", "julian", "female_loading", "year_f")
...
spatial_grid_mat_female$female_loading <- median(mat_female_train$female_loading, na.rm = TRUE)
...
## Immature females ----
# Get best models
brt_imm_female_base <- grid_search(imm_female_train, 11, 'bernoulli')
brt_imm_female_base
...
rmse_imm_female_brt # 1.44

# Calculate deviance
dev_imm_female_abun <- brt_deviance(brt_imm_female_abun)
dev_imm_female_pres <- brt_deviance(brt_imm_female_base)

dev_imm_female_abun # 61% deviance explained
dev_imm_female_pres # 52% deviance explained

# Save models for future use
saveRDS(brt_imm_female_abun, file = here('data', 'brt_imm_female_abun_for.rds'))
saveRDS(brt_imm_female_base, file = here('data', 'brt_imm_female_base_for.rds'))

# Read in BRTs
brt_imm_female_abun <- readRDS(file = here('data', 'brt_imm_female_abun_for.rds'))
brt_imm_female_base <- readRDS(file = here('data', 'brt_imm_female_base_for.rds'))

# Variable importance
rel_inf(brt_imm_female_abun, 'Variable Influence on Immature Female Snow Crab')
dev.copy(jpeg,
         here('results/BRT',
              'female_imm_rel_inf_for.jpg'),
         height = 12,
         width = 9,
         res = 200,
         units = 'in')
...
saveRDS(brt_leg_male_base, file = here('data', 'brt_leg_male_base_for.rds'))

# Read in BRTs
brt_leg_male_abun <- readRDS(file = here('data', 'brt_leg_male_abun_for.rds'))
brt_leg_male_base <- readRDS(file = here('data', 'brt_leg_male_base_for.rds'))

# Variable importance
rel_inf(brt_leg_male_abun, 'Variable Influence on Legal Male Snow Crab')
dev.copy(jpeg,
         here('results/BRT',
              'male_leg_rel_inf_for.jpg'),
         height = 12,
         width = 9,
         res = 200,
         units = 'in')
...
saveRDS(brt_sub_male_base, file = here('data', 'brt_sub_male_base_for.rds'))

# Read in BRTs
brt_sub_male_abun <- readRDS(file = here('data', 'brt_sub_male_abun_for.rds'))
brt_sub_male_base <- readRDS(file = here('data', 'brt_sub_male_base_for.rds'))
